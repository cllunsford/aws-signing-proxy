...
func handler(proxy *httputil.ReverseProxy) func(http.ResponseWriter, *auth.AuthenticatedRequest) {
    return func(writer http.ResponseWriter, request *auth.AuthenticatedRequest) {
        proxy.ServeHTTP(writer, &request.Request)
    }
}
...
func main() {
    ...
    authenticator := auth.NewBasicAuthenticator("kibana.admetricks.net", Secret)
